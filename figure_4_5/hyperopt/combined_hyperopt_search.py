"""
SEARCH OF HYPERPARAMETER FOR COMBINED DATASET 
(Kinker et al and Kim et al)
...
Launched on Rossmann to enjoy GPU acceleration.
"""
...
combined_folder = '../data/Kinker_Kim/'
...
save_id = 'combined_' + '{:%B_%d_%Y_}'.format(datetime.datetime.now()) + str(uuid.uuid4()).split('-')[1]
...
# Read data
combined_data_file = '%s/%s/combined_count.csv'%(combined_folder, 
                                                data_subfolder)
combined_data_df = pd.read_csv(combined_data_file, sep=',', index_col=[0,1,2,3])
combined_data_df.index.names = ['UMI', 'sample', 'pool', 'type']

# Format and divide test and train
train_data_df, test_data_df = train_test_split(combined_data_df, test_size=test_size)
train_data_an = AnnData(
    train_data_df.values, 
    obs=pd.DataFrame(np.array([np.array(e) for e in train_data_df.index]),
                     columns=['UMI', 'sample', 'pool', 'type'])
)
...
def _objective_function(params):
    ...
    clf = scvi.model.SCVI(
        ...
        dispersion=dispersion, gene_likelihood=likelihood
    )
