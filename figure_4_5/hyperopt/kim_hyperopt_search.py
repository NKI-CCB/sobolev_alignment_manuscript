"""
SEARCH OF HYPERPARAMETER FOR KIM ET AL
2021/04/23 - Soufiane Mourragui
...
save_id = 'tumor_' + "{:%B_%d_%Y_}".format(datetime.datetime.now()) + str(uuid.uuid4()).split('-')[1]
...
# Read data
tumor_data_file = '%s/%s/tumor_count.csv'%(tumor_folder, 
                                        data_subfolder)
tumor_data_df = pd.read_csv(tumor_data_file, sep=',', index_col=[0,1,2])
tumor_data_df.index.names = ['UMI', 'sample', 'pool']

# Format and divide test and train
train_data_df, test_data_df = train_test_split(tumor_data_df, test_size=test_size)
