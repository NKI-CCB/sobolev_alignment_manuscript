"""
This script performs the null model by:
...
# Import params
from model_I_synthetic_params import *
...
print('START NULL MODEL', flush=True)
...
for nu in [source_krr_params['kernel_params']['nu'], np.inf]:
    results_file = '%s/null_model_%s_perm_%s.csv'%(
        output_folder,
        'laplacian' if nu == 0.5 else ('gaussian' if nu == np.inf else nu),
        n_perm
    )
    if results_file.split('/')[1] is os.listdir(output_folder):
        continue
    ...
    for k in permuted_sobolev_alignment_clf.krr_params:
        ...
        permuted_sobolev_alignment_clf.krr_params[k]['M'] = null_model_M
    ...
    for perm_idx in range(n_perm):
        ...
        print('\t START SHUFFLING', flush=True)
        ...
        for data_source in permuted_sobolev_alignment_clf.artificial_embeddings_:
            permuted_sobolev_alignment_clf.artificial_embeddings_[data_source] = shuffle_matrix(
                permuted_sobolev_alignment_clf.artificial_embeddings_[data_source]
            )

        # Train KRR on gene-level permuted data
        print('\t START SOBOLEV ALIGNMENT', flush=True)
    ...
    null_model_sim_df.to_csv(results_file)

gc.collect()
sys.exit("FINISH NULL MODEL PV SIM")