...
if 'hyperopt_target' not in os.listdir(output_folder):
    os.mkdir('%s/hyperopt_target'%(output_folder))
elif len(os.listdir('%s/hyperopt_target'%(output_folder))) > 0:
    sys.exit("HYPEROPT ALREADY CARRIED OUT ON TARGET")
output_folder = '%s/hyperopt_target'%(output_folder)

save_id = 'target_' + "{:%B_%d_%Y_}".format(datetime.datetime.now()) + str(uuid.uuid4()).split('-')[1]
...
_, X_target = read_data(data_folder, data_subfolder)
target_data_df = X_target.to_df()

# Format and divide test and train
train_data_df, test_data_df = train_test_split(target_data_df, test_size=test_size)
