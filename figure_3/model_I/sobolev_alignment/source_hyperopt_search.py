...
if 'hyperopt_source' not in os.listdir(output_folder):
    os.mkdir('%s/hyperopt_source'%(output_folder))
elif len(os.listdir('%s/hyperopt_source'%(output_folder))) > 0:
    sys.exit("HYPEROPT ALREADY CARRIED OUT ON SOURCE")
output_folder = '%s/hyperopt_source'%(output_folder)

save_id = 'source_' + '{:%B_%d_%Y_}'.format(datetime.datetime.now()) + str(uuid.uuid4()).split('-')[1]
...
space = {
    ...
    'n_latent': 2 + hp.randint('n_latent', 10),
...
X_source, _ = read_data(data_folder, data_subfolder)
source_data_df = X_source.to_df()

# Format and divide test and train
train_data_df, test_data_df = train_test_split(source_data_df, test_size=test_size)
...
scvi.data.setup_anndata(train_data_an, batch_key=None)
