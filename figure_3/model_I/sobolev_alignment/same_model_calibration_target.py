...
sobolev_alignment_clf.scvi_models['source'] = sobolev_alignment_clf.scvi_models['target']
gc.collect()
...
# Folder for saving
if 'comparison_same_model_target' not in os.listdir(output_folder):
    os.mkdir('%s/comparison_same_model_target'%(output_folder))
output_folder = '%s/comparison_same_model_target/'%(output_folder)
...
for krr_params in krr_param_grid: 
    param_id = 'target_kernel_%s_M_%s_penalization_%s_params_%s_maxiter_%s'%(
        krr_params['kernel'],
        krr_params['M'],
    ...
    pd.DataFrame(principal_angles_df).to_csv('%s/target_principal_angles.csv'%(output_folder))
